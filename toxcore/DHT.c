                          ...
                          const uint8_t *sendback_data, uint16_t length, const uint8_t *shared_encryption_key)
{
    ...
    size_t data_len = 1 + crypto_box_PUBLICKEYBYTES + crypto_box_NONCEBYTES + Node_format_size * MAX_SENT_NODES + length + crypto_box_MACBYTES;
    ...
    size_t plain_size = 1 + Node_format_size * MAX_SENT_NODES + length;
    uint8_t * plain = malloc(plain_size);

    if (!plain)
        return -1;

    size_t encrypt_len = plain_size + crypto_box_MACBYTES;
    uint8_t * encrypt = malloc(encrypt_len);
    ...
    result = sendpacket(dht->net, ip_port, data, 1 + crypto_box_PUBLICKEYBYTES + crypto_box_NONCEBYTES + len);
                                 ...
                                 Node_format *plain_nodes, uint16_t size_plain_nodes, uint32_t *num_nodes_out)
{
    ...
    size_t plain_size = 1 + data_size + sizeof(uint64_t);
    ...
    if ((unsigned int)len != plain_size) {
      result = 1;
      goto extra_exit;
    }
        ...
        Client_data *list, uint32_t list_count, uint32_t *bootstrap_times, _Bool sortable)
{
    ...
    Client_data **client_list = malloc((list_count * 2) * sizeof(Client_data *));
    IPPTsPng    **assoc_list = malloc((list_count * 2) * sizeof(IPPTsPng *));
                                      ...
                                      const uint8_t *nodes_data, uint16_t nodes_data_length)
{
    ...
    size_t data_len = 1 + crypto_box_PUBLICKEYBYTES + nodes_data_length;
    uint8_t * data = malloc(data_len);
    ...
    int len = create_request(dht->self_public_key, dht->self_secret_key, packet, sendto->public_key, data,
      data_len, CRYPTO_PACKET_HARDENING);
    ...
    result = sendpacket(dht->net, sendto->ip_port, packet, len);
    free(data);
