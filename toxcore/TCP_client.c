...
static int proxy_http_generate_connection_request(TCP_Client_Connection *TCP_conn)
{
    ...
    const int written = _snprintf_s((char *)TCP_conn->last_packet, MAX_PACKET_SIZE, MAX_PACKET_SIZE, "%s%s:%hu%s%s:%hu%s", one, ip, port, two,
                                 ip, port, three);
...
static int proxy_http_read_connection_response(TCP_Client_Connection *TCP_conn)
{
    ...
    if (strstr((char *)data, success)) {
        ...
        if (data_left) {
            uint8_t * temp_data = malloc(data_left);
        ...
        _Bool priority)
{
    ...
    size_t packet_len = sizeof(uint16_t) + length + crypto_box_MACBYTES;
    ...
    if ((unsigned int)len != (packet_len - sizeof(uint16_t))) {
        free(packet);
        return -1;
    }

    if (priority) {
        len = sendpriority ? send(con->sock, packet, packet_len, MSG_NOSIGNAL) : 0;
        ...
        if ((unsigned int)len == packet_len) {
            free(packet);
            return 1;
        }

        return add_priority(con, packet, packet_len, len);
    }

    len = send(con->sock, packet, packet_len, MSG_NOSIGNAL);
    ...
    memcpy(con->last_packet, packet, packet_len);
    con->last_packet_length = packet_len;
    con->last_packet_sent = len;
...
int send_data(TCP_Client_Connection *con, uint8_t con_id, const uint8_t *data, uint16_t length)
{
    ...
    int result = write_packet_TCP_secure_connection(con, packet, packet_len, 0);
...
int send_oob_packet(TCP_Client_Connection *con, const uint8_t *public_key, const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = 1 + crypto_box_PUBLICKEYBYTES + length;
    uint8_t * packet = malloc(packet_len);
