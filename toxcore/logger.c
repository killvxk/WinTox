...
Logger *logger_new (const char *file_name, LOG_LEVEL level, const char *id)
{
    ...
    if (!fopen_s(&retu->log_file, file_name, "ab")) {
    ...
    if (id) {
        ...
        _snprintf_s(retu->id, 8, 8, "%u", random_int());
    }
...
void logger_write (Logger *log, LOG_LEVEL level, const char *file, int line, const char *format, ...)
{
    ...
    /* Set position str */
    _snprintf_s(this_log->posstr, 300, 300, "%s:%d", SFILE(file), line);

    /* Set message */
    va_list args;
    va_start (args, format);
    vsnprintf_s(this_log->msg, 4096, 4096, format, args);
    va_end (args);
