...
static int handle_announce_response(void *object, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    size_t plain_len = 1 + ONION_PING_ID_SIZE + len_nodes;
...
static int handle_data_response(void *object, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    size_t temp_plain_len = length - ONION_DATA_RESPONSE_MIN_SIZE;
    uint8_t * temp_plain = malloc(temp_plain_len);
    ...
    if ((uint32_t)len != temp_plain_len) {
        free(temp_plain);
        return 1;
    }

    size_t plain_len = temp_plain_len - DATA_IN_RESPONSE_MIN_SIZE;
    uint8_t * plain = malloc(plain_len);

    len = decrypt_data(temp_plain, onion_c->c->self_secret_key, packet + 1, temp_plain + crypto_box_PUBLICKEYBYTES,
      temp_plain_len - crypto_box_PUBLICKEYBYTES, plain);
    ...
    result = onion_c->Onion_Data_Handlers[plain[0]].function(onion_c->Onion_Data_Handlers[plain[0]].object, temp_plain, plain, plain_len);
...
int send_onion_data(Onion_Client *onion_c, int friend_num, const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = DATA_IN_RESPONSE_MIN_SIZE + length;
    ...
    if ((uint32_t)len + crypto_box_PUBLICKEYBYTES != packet_len) {
        free(packet);
        return -1;
    }
    ...
    for (i = 0; i < num_good; ++i) {
        ...
        len = create_data_request(o_packet, sizeof(o_packet), onion_c->friends_list[friend_num].real_public_key,
          list_nodes[good_nodes[i]].data_public_key, nonce, packet, packet_len);
...
static int send_dht_dhtpk(const Onion_Client *onion_c, int friend_num, const uint8_t *data, uint16_t length)
{
    ...
    size_t temp_len = DATA_IN_RESPONSE_MIN_SIZE + crypto_box_NONCEBYTES + length;
    ...
    if ((uint32_t)len + crypto_box_PUBLICKEYBYTES + crypto_box_NONCEBYTES != temp_len) {
        result = -1;
        goto exit_mark;
    }
    ...
    len = create_request(onion_c->dht->self_public_key, onion_c->dht->self_secret_key, packet,
      onion_c->friends_list[friend_num].dht_public_key, temp, temp_len, CRYPTO_PACKET_DHTPK);
    ...
    result = route_tofriend(onion_c->dht, onion_c->friends_list[friend_num].dht_public_key, packet, len);
...
static int onion_isconnected(const Onion_Client *onion_c)
{
    ...
    if (is_timeout(onion_c->last_packet_recv, (uint64_t)ONION_OFFLINE_TIMEOUT))
        return 0;
