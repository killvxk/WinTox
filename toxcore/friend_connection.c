                               ...
                               uint16_t length)
{
    ...
    size_t packet_len = 1 + sizeof(nospam_num) + length;
    ...
    if (friend_con->status == FRIENDCONN_STATUS_CONNECTED) {
        ...
        result = write_cryptpacket(fr_c->net_crypto, friend_con->crypt_connection_id, packet, packet_len, 0) != -1;
        goto extra_exit;
    } else {
        packet[0] = CRYPTO_PACKET_FRIEND_REQ;
        int num = send_onion_data(fr_c->onion_c, friend_con->onion_friendnum, packet, packet_len);
