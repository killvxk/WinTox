                        ...
                        const uint8_t *data, uint16_t length)
{
    ...
    size_t step1_len = SIZE_IPPORT + length;
    uint8_t * step1 = malloc(step1_len);
    ...
    size_t step2_len = SIZE_IPPORT + SEND_BASE + length;
    uint8_t * step2 = malloc(step2_len);
    ipport_pack(step2, &path->ip_port3);
    memcpy(step2 + SIZE_IPPORT, path->public_key3, crypto_box_PUBLICKEYBYTES);

    int len = encrypt_data_symmetric(path->shared_key3, nonce, step1, step1_len,
    ...
    size_t step3_len = SIZE_IPPORT + SEND_BASE * 2 + length;
    uint8_t * step3 = malloc(step3_len);
    ipport_pack(step3, &path->ip_port2);
    memcpy(step3 + SIZE_IPPORT, path->public_key2, crypto_box_PUBLICKEYBYTES);
    len = encrypt_data_symmetric(path->shared_key2, nonce, step2, step2_len,
    ...
    len = encrypt_data_symmetric(path->shared_key1, nonce, step3, step3_len,
    ...
    result = 1 + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES + len;
                            ...
                            const uint8_t *data, uint16_t length)
{
    ...
    result = crypto_box_NONCEBYTES + SIZE_IPPORT + crypto_box_PUBLICKEYBYTES + len;
...
int send_onion_response(Networking_Core *net, IP_Port dest, const uint8_t *data, uint16_t length, const uint8_t *ret)
{
    ...
    size_t packet_len = 1 + RETURN_3 + length;
