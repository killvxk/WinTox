...
const char *ip_ntoa(const IP *ip)
{
    if (ip) {
        if (ip->family == AF_INET) {
          ...
          _snprintf_s(addresstext, sizeof(addresstext), sizeof(addresstext), "(IP invalid, family %u)", ip->family);
    } else
      _snprintf_s(addresstext, sizeof(addresstext), sizeof(addresstext), "(IP invalid: NULL)");
