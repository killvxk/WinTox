                                      ...
                                      uint8_t *recv_nonce, uint8_t *data, uint16_t max_len)
{
    ...
    uint8_t * data_encrypted = malloc(*next_packet_length);
        ...
        _Bool priority)
{
    ...
    if (priority) {
        ...
        result = add_priority(con, packet, packet_len, len);
        goto extra_exit;
    }
                               ...
                               uint16_t length)
{
    ...
    if (other_index != -1) {
        size_t resp_packet_len = 1 + crypto_box_PUBLICKEYBYTES + length;
        uint8_t * resp_packet = malloc(resp_packet_len);
...
static int handle_onion_recv_1(void *object, IP_Port dest, const uint8_t *data, uint16_t length)
{
    ...
    if (write_packet_TCP_secure_connection(con, packet, packet_len, 0) != 1)
        return 1;
