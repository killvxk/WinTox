...
static int setnick(Group_Chats *g_c, int groupnumber, int peer_index, const uint8_t *nick, uint16_t nick_len)
{
    ...
    g->group[peer_index].nick_len = (uint8_t)nick_len;
        ...
        uint16_t group_num, const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = 1 + sizeof(uint16_t) + length;
    uint8_t * packet = malloc(packet_len);
    packet[0] = packet_id;
    memcpy(packet + 1, &group_num, sizeof(uint16_t));
    memcpy(packet + 1 + sizeof(uint16_t), data, length);

    result = write_cryptpacket(fr_c->net_crypto, friend_connection_crypt_connection_id(fr_c, friendcon_id), packet,
      packet_len, 0) != -1;
        ...
        uint16_t group_num, const uint8_t *data, uint16_t length)
{
    ...
    result = send_lossy_cryptpacket(fr_c->net_crypto, friend_connection_crypt_connection_id(fr_c, friendcon_id), packet, packet_len) != -1;
    free(packet);
...
static unsigned int send_peers(Group_Chats *g_c, int groupnumber, int friendcon_id, uint16_t group_num)
{
    ...
    if (g->title_len) {
        size_t Packet_len = 1 + g->title_len;
        uint8_t * Packet = malloc(Packet_len);
        Packet[0] = PEER_TITLE_ID;
        memcpy(Packet + 1, g->title, g->title_len);
        send_packet_group_peer(g_c->fr_c, friendcon_id, PACKET_ID_DIRECT_GROUPCHAT, group_num, Packet, Packet_len);
        free(Packet);
    }
                                       ...
                                       uint16_t len)
{
    ...
    result = send_message_all_close(g_c, groupnumber, packet, packet_len, -1);
...
int send_group_lossy_packet(const Group_Chats *g_c, int groupnumber, const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = sizeof(uint16_t) * 2 + length;
    uint8_t * packet = malloc(packet_len);
                                        ...
                                        int close_index)
{
    ...
    switch (message_id) {
        ...
        case GROUP_MESSAGE_TITLE_ID: {
            if (settitle(g_c, groupnumber, index, msg_data, (uint8_t)msg_data_len) == -1)
                return;
        }
        break;
        ...
        case PACKET_ID_MESSAGE: {
            ...
            size_t newmsg_len = msg_data_len + 1;
            uint8_t * newmsg = malloc(newmsg_len);
            memcpy(newmsg, msg_data, msg_data_len);
            newmsg[msg_data_len] = 0;

            //TODO
            if (g_c->message_callback)
                g_c->message_callback(g_c->m, groupnumber, index, newmsg, (uint8_t)msg_data_len, g_c->message_callback_userdata);
