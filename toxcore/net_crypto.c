...
static int send_data_packet(Net_Crypto *c, int crypt_connection_id, const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = 1 + sizeof(uint16_t) + length + crypto_box_MACBYTES;
    ...
    if (len + 1 + sizeof(uint16_t) != packet_len) {
        pthread_mutex_unlock(&conn->mutex);
        free(packet);
        return -1;
    }
    ...
    result = send_packet_to(c, crypt_connection_id, packet, packet_len);
    free(packet);
                                   ...
                                   const uint8_t *data, uint16_t length)
{
    ...
    size_t packet_len = sizeof(uint32_t) + sizeof(uint32_t) + padding_length + length;
    uint8_t * packet = malloc(packet_len);
    ...
    result = send_data_packet(c, crypt_connection_id, packet, packet_len);
                                    ...
                                    uint8_t congestion_control)
{
    ...
    if (send_data_packet_helper(c, crypt_connection_id, conn->recv_array.buffer_start, (uint32_t)packet_num, data, length) == 0) {
        Packet_Data *dt1 = NULL;

        if (get_data_pointer(&conn->send_array, &dt1, (uint32_t)packet_num) == 1)
                                     ...
                                     _Bool udp)
{
    ...
    if (real_data[0] == PACKET_ID_REQUEST) {
        ...
        while (1) {
            pthread_mutex_lock(&conn->mutex);
            int ret = (int)read_data_beg_buffer(&conn->recv_array, &dt);
            pthread_mutex_unlock(&conn->mutex);
...
static void send_crypto_packets(Net_Crypto *c)
{
        ...
        if (conn->status == CRYPTO_CONN_ESTABLISHED) {
            if (conn->packet_recv_rate > CRYPTO_PACKET_MIN_RATE) {
                ...
                if (request_packet_interval < peak_request_packet_interval) {
                    peak_request_packet_interval = (uint32_t)request_packet_interval;
                }
            }

            if ((PACKET_COUNTER_AVERAGE_INTERVAL + conn->packet_counter_set) < temp_time) {

                double dt = (double)(temp_time - conn->packet_counter_set);
            ...
            if (conn->last_packets_left_set == 0 || conn->last_packets_left_requested_set == 0) {
                ...
                if (((uint64_t)((1000.0 / conn->packet_send_rate) + 0.5) + conn->last_packets_left_set) <= temp_time) {
                    ...
                    uint32_t num_packets = (uint32_t)n_packets;
    ...
    if (total_send_rate > CRYPTO_PACKET_MIN_RATE) {
        sleep_time = (uint32_t)(1000.0 / total_send_rate);
