                           ...
                           uint32_t *message_id)
{
    ...
    add_receipt(m, friendnumber, (uint32_t)packet_num, msg_id);
                                ...
                                uint32_t length, uint8_t congestion_control)
{
    ...
    int result = write_cryptpacket(m->net_crypto, friend_connection_crypt_connection_id(m->fr_c,
                            ...
                            uint64_t filesize, const uint8_t *file_id, const uint8_t *filename, uint16_t filename_length)
{
    ...
    size_t packet_len = 1 + sizeof(file_type) + sizeof(filesize) + FILE_ID_LENGTH + filename_length;
    ...
    int result = write_cryptpacket_id(m, friendnumber, PACKET_ID_FILE_SENDREQUEST, packet, packet_len, 0);
    free(packet);
                             ...
                             uint8_t control_type, uint8_t *data, uint16_t data_length)
{
    ...
    size_t packet_len = 3 + data_length;
    uint8_t * packet = malloc(packet_len);
    ...
    int result = write_cryptpacket_id(m, friendnumber, PACKET_ID_FILE_CONTROL, packet, packet_len, 0);
    free(packet);
                                     ...
                                     uint16_t length)
{
    ...
    int result = (int)write_cryptpacket(m->net_crypto, friend_connection_crypt_connection_id(m->fr_c, m->friendlist[friendnumber].friendcon_id), packet, (uint16_t)packet_len, 1);
    free(packet);
              ...
              uint16_t length)
{
    ...
    if (ret != -1) {
        ...
        if (length != MAX_FILE_DATA_SIZE || ft->size == ft->transferred) {
            ...
            ft->last_packet_number = (uint32_t)ret;
        }
    ...
    for (i = 0; i < MAX_CONCURRENT_FILE_PIPES; ++i) {
        ...
        while (ft->status == FILESTATUS_TRANSFERRING && (ft->paused == FILE_PAUSE_NOT)) {
            ...
            if (ft->size - ft->requested < length) {
                length = (uint16_t)(ft->size - ft->requested);
            }
...
static int handle_packet(void *object, int i, uint8_t *temp, uint16_t len)
{
    ...
    switch (packet_id) {
        ...
        case PACKET_ID_NICKNAME: {
            ...
            size_t data_terminated_len = data_length + 1;
            uint8_t * data_terminated = malloc(data_terminated_len);
        ...
        case PACKET_ID_ACTION: {
            ...
            /* Make sure the NULL terminator is present. */
            size_t message_terminated_len = message_length + 1;
            uint8_t * message_terminated = malloc(message_terminated_len);
        ...
        case PACKET_ID_FILE_SENDREQUEST: {
            ...
            size_t filename_terminated_len = filename_length + 1;
            uint8_t * filename_terminated = malloc(filename_terminated_len);
        ...
        case PACKET_ID_FILE_DATA: {
            ...
            if ((ft->transferred + file_data_length) > ft->size) {
                file_data_length = (uint16_t)(ft->size - ft->transferred);
            }
